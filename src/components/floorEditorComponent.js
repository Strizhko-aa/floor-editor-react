...
class FloorEditor extends React.Component {
  ...
  removeEditableObject () {
    this.state.fe.removeEditableObject()
  }
  ...
  render() {
    ...
    let appControl = (<div/>)
    if (_mode === 'editor') {
        appControl = (<div className="app-control" id={'control_' + this.state.blockForMap}>
        ...
        <button className="remove-drawing" title="Удалить редактируемый объект" onClick={() => {this.removeEditableObject()}}>
